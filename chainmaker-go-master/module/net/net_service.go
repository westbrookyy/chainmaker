...
func (ns *NetService) consensusBroadcastMsg(msg []byte, topic string) error {
	...
	ns.logger.Infof("[NetService] consensusBroadcastMsg: got %d consensus nodes, topic: %s, msg size: %d", 
		len(consensusNodeIdList), topic, len(msg))
	if len(consensusNodeIdList) == 0 {
		ns.logger.Warnf("[NetService] consensusBroadcastMsg: consensus node list is empty, cannot broadcast")
		return nil
	}
	ns.logger.Debugf("[NetService] consensusBroadcastMsg: consensus nodes: %v", consensusNodeIdList)
	var wg sync.WaitGroup
	wg.Add(len(consensusNodeIdList))
	for i := range consensusNodeIdList {
		to := consensusNodeIdList[i]
		if to == ns.localNet.GetNodeUid() {
			ns.logger.Debugf("[NetService] consensusBroadcastMsg: skipping self (%s)", to)
			wg.Done()
			continue
		}
		toNode := to // Capture loop variable
		go func() {
			defer wg.Done()
			ns.logger.Debugf("[NetService] consensusBroadcastMsg: sending to %s (topic: %s, size: %d)", 
				toNode, topic, len(msg))
			if err := ns.localNet.SendMsg(ns.chainId, toNode, topic, msg); err != nil {
				ns.logger.Warnf("[NetService] send consensus broadcast msg failed (to: %s, topic: %s), %s", 
					toNode, topic, err.Error())
			} else {
				ns.logger.Debugf("[NetService] consensusBroadcastMsg: successfully sent to %s", toNode)
			}
		}()
	}
	wg.Wait()
	ns.logger.Debugf("[NetService] consensusBroadcastMsg: completed broadcasting to all nodes")
	return nil
}
...
func (n *NetContractEventSubscribe) onMessageChainConfig(msg *msgbus.Message) {
	...
	oldCount := len(n.ns.consensusNodeIds)
	n.ns.consensusNodeIds = newConsensusNodeIds
	newCount := len(newConsensusNodeIds)
	n.ns.consensusNodeIdsLock.Unlock()
	
	// Log consensus node IDs for debugging
	nodeIdList := make([]string, 0, newCount)
	for nodeId := range newConsensusNodeIds {
		nodeIdList = append(nodeIdList, nodeId)
	}
	n.ns.logger.Infof("[NetService] refresh ids of consensus nodes: old count=%d, new count=%d, nodes=%v", 
		oldCount, newCount, nodeIdList)
	// 2.re-verify peers
	n.ns.localNet.ReVerifyPeers(n.ns.chainId)
...
func HandleMsgBusSubscriberOnMessage(
	...
	netService.logger.Debugf("[NetService/msg-bus %s subscriber] HandleMsgBusSubscriberOnMessage called", logMsgDescription)
	if netMsg, ok := message.Payload.(*netPb.NetMsg); ok {
		netService.logger.Debugf("[NetService/msg-bus %s subscriber] got NetMsg: type=%v, to='%s', payload size=%d", 
			logMsgDescription, netMsg.Type, netMsg.To, len(netMsg.GetPayload()))
		...
		if netMsg.To == "" {
			netService.logger.Debugf("[NetService/msg-bus %s subscriber] To is empty, using broadcast", logMsgDescription)
			return handleMsgBusSubscriberOnMessageBroadcast(netService, msgType, logMsgDescription, netMsg)
		}

		netService.logger.Debugf("[NetService/msg-bus %s subscriber] To is '%s', using send", logMsgDescription, netMsg.To)
		return handleMsgBusSubscriberOnMessageSend(netService, msgType, logMsgDescription, netMsg)
	}
	netService.logger.Warnf("[NetService/msg-bus %s subscriber] failed to assert message.Payload to *netPb.NetMsg", logMsgDescription)
	return nil
}
...
func handleMsgBusSubscriberOnMessageBroadcast(
	...
	netService.logger.Infof("[NetService/msg-bus %s subscriber] handleMsgBusSubscriberOnMessageBroadcast: msgType=%v, payload size=%d", 
		logMsgDescription, msgType, len(netMsg.GetPayload()))
	
	isConsensusMsg := (msgType == netPb.NetMsg_TX || msgType == netPb.NetMsg_CONSENSUS_MSG)
	isConsensusNodeListEmpty := netService.isConsensusNodeIdListEmpty()
	
	netService.logger.Debugf("[NetService/msg-bus %s subscriber] isConsensusMsg=%v, isConsensusNodeListEmpty=%v", 
		logMsgDescription, isConsensusMsg, isConsensusNodeListEmpty)
	
	if isConsensusMsg && !isConsensusNodeListEmpty {
		topic := CreateFlagWithPrefixAndMsgType(msgBusConsensusTopicPrefix, msgType)
		netService.logger.Infof("[NetService/msg-bus %s subscriber] using consensusBroadcastMsg with topic: %s", 
			logMsgDescription, topic)
		if err := netService.consensusBroadcastMsg(
			netMsg.GetPayload(),
			topic,
		); err != nil {
			netService.logger.Warnf(
				"[NetService/msg-bus %s subscriber] consensusBroadcastMsg failed, %s",
				logMsgDescription,
				err.Error(),
			)
			return err
		...
		topic := CreateFlagWithPrefixAndMsgType(msgBusTopicPrefix, msgType)
		netService.logger.Infof("[NetService/msg-bus %s subscriber] using broadcastMsg with topic: %s", 
			logMsgDescription, topic)
		if err := netService.broadcastMsg(
			...
			netService.logger.Warnf(
				"[NetService/msg-bus %s subscriber] broadcastMsg failed, %s",
				logMsgDescription,
				err.Error(),
			)
			return err
	...
	netService.logger.Infof("[NetService/msg-bus %s subscriber] broadcast ok", logMsgDescription)
	return nil
}
...
func CreateMsgHandlerForMsgBus(
	...
	return func(chainId string, node string, data []byte) error {
		netService.logger.Infof("[NetService/%s handler for msg-bus] receive msg (size:%d) (from:%s, chainId:%s)",
			logMsgDescription,
			len(data),
			node,
			chainId,
		...
		if netService.chainId != chainId {
			netService.logger.Warnf("[NetService/%s handler for msg-bus] wrong chain-id(expected:%s, got:%s), ignored.",
				logMsgDescription,
				netService.chainId,
				chainId,
			)
		...
		netService.logger.Debugf("[NetService/%s handler for msg-bus] publishing to msgbus topic: %v", 
			logMsgDescription, topic)
		netService.msgBus.Publish(topic, pbMsg)
		netService.logger.Debugf("[NetService/%s handler for msg-bus] message published successfully", logMsgDescription)
		return nil
	}
}
