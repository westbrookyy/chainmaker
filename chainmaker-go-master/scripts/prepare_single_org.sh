...
function show_help() {
    ...
    echo "               -c consense-type: 0-SOLO,1-TBFT,3-MAXBFT,4-RAFT,7-PBFT "
    ...
    echo "    eg4: v 11301 12301 32351 22351 -c 1 -l INFO -v false"
}
...
function generate_config() {
    ...
    if [ $CONSENSUS_TYPE == -1 ] ;then
      read -p "input consensus type (0-SOLO,1-TBFT(default),3-MAXBFT,4-RAFT,7-PBFT): " tmp
      if  [ ! -z "$tmp" ] ;then
        if  [ $tmp -eq 0 ] || [ $tmp -eq 1 ] || [ $tmp -eq 3 ] || [ $tmp -eq 4 ] || [ $tmp -eq 7 ] ;then
          CONSENSUS_TYPE=$tmp
        else
    ...
    if [ $CONSENSUS_TYPE == 7 ] && [ $NODE_CNT -lt 4 ] ;then
      echo  "PBFT consensus requires at least 4 nodes (3f+1 for fault tolerance)"
      exit
    fi
