...
type timeScheduler struct {
	...
	id       string
	timer    *time.Timer
	bufferC  chan pbftpb.TimeoutInfo
	timeoutC chan pbftpb.TimeoutInfo
	stopC    chan struct{}
}

// newTimeScheduler returns a new timeScheduler
func newTimeScheduler(logger protocol.Logger, id string) *timeScheduler {
	ts := &timeScheduler{
		logger:   logger,
		id:       id,
		timer:    time.NewTimer(0),
		bufferC:  make(chan pbftpb.TimeoutInfo, defaultTimeSchedulerBufferSize),
		timeoutC: make(chan pbftpb.TimeoutInfo, defaultTimeSchedulerBufferSize),
...
func (ts *timeScheduler) AddTimeoutInfo(ti pbftpb.TimeoutInfo) {
	ts.logger.Debugf("len(ts.bufferC): %d", len(ts.bufferC))
	ts.bufferC <- ti
}

// GetTimeoutC returns timeoutC for consuming
func (ts *timeScheduler) GetTimeoutC() <-chan pbftpb.TimeoutInfo {
	return ts.timeoutC
}
...
func (ts *timeScheduler) handle() {
	...
	var ti pbftpb.TimeoutInfo // latest timeout event had been seen
	for {
		select {
		case t := <-ts.bufferC:
