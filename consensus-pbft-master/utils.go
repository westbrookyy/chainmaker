...
import (
	...
	"chainmaker.org/chainmaker/pb-go/v2/consensus/pbft"
...
func VerifyBlockSignatures(chainConf protocol.ChainConf,
	...
	// TODO: This function needs to be refactored to use the new PBFT types
	// The old VoteSet type is no longer available in the protobuf definitions
	// For now, we'll use CommitVoteSet which is the current type for storing votes
...
func verifyPrepareVotes(votes map[string]*pbft.Prepare, ac protocol.AccessControlProvider, blockVersion uint32) error {
	if votes == nil {
	...
	for _, value := range votes {
		...
		go func(v *pbftpb.Prepare) {
			...
			prepare, ok := proto.Clone(v).(*pbft.Prepare)
			if !ok {
				err = fmt.Errorf("interface transfer to *pbft.Prepare failed")
				return
			}
			prepare.Endorsement = nil
			message := mustMarshal(prepare)
...
func verifyCommitVotes(votes map[string]*pbft.Commit, ac protocol.AccessControlProvider, blockVersion uint32) error {
	if votes == nil {
		return errors.New("invalid votes")
	}
	...
	for _, value := range votes {
		...
		go func(v *pbftpb.Commit) {
			...
			commit, ok := proto.Clone(v).(*pbft.Commit)
			if !ok {
				err = fmt.Errorf("interface transfer to *pbft.Commit failed")
				return
			}
			commit.Endorsement = nil
			message := mustMarshal(commit)
