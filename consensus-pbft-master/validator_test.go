...
// TestValidatorSet_GetByIndex_Nil 测试nil集合调用getByIndex
func TestValidatorSet_GetByIndex_Nil(t *testing.T) {
	var nilVs *validatorSet
	_, err := nilVs.getByIndex(0)
	require.NotNil(t, err)
	require.Equal(t, ErrInvalidIndex, err)
}

// TestValidatorSet_GetIndexByString_Nil 测试nil集合调用getIndexByString
func TestValidatorSet_GetIndexByString_Nil(t *testing.T) {
	var nilVs *validatorSet
	index := nilVs.getIndexByString("node1")
	require.Equal(t, int32(-1), index)
}

// TestValidatorSet_UpdateValidators_Cleanup 测试更新validators时清理关联数据
func TestValidatorSet_UpdateValidators_Cleanup(t *testing.T) {
	validators := []string{"node1", "node2", "node3"}
	vs := newValidatorSet(newTestLogger(), validators)

	// 移除node2和node3
	newValidators := []string{"node1"}
	added, removed, err := vs.updateValidators(newValidators)
	require.Nil(t, err)
	require.Equal(t, 0, len(added))
	require.Equal(t, 2, len(removed))
	require.Contains(t, removed, "node2")
	require.Contains(t, removed, "node3")

	// 验证关联数据已被清理（validatorsHeight和validatorsBeatTime）
	// 注意：这些字段目前是保留字段，但代码已经添加了清理逻辑
	require.Equal(t, 1, len(vs.Validators))
	require.Equal(t, "node1", vs.Validators[0])
}
