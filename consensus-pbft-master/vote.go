...
func NewPrepareVoteSet(logger protocol.Logger, view, sequence uint64, digest []byte,
	...
	if logger == nil {
		// logger should not be nil, but handle gracefully
		return nil
	}
...
func (pvs *PrepareVoteSet) AddVote(prepare *pbftpb.Prepare) (added bool, err error) {
	if pvs == nil {
		return false, fmt.Errorf("PrepareVoteSet is nil")
	}
	...
	if pvs.validators == nil {
		return false, fmt.Errorf("validators is nil")
	}
...
func (cvs *CommitVoteSet) AddVote(commit *pbftpb.Commit) (added bool, err error) {
	if cvs == nil {
		return false, fmt.Errorf("CommitVoteSet is nil")
	}
