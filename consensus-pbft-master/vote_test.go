...
// TestPrepareVoteSet_AddVote_NilValidators 测试nil validators的情况
func TestPrepareVoteSet_AddVote_NilValidators(t *testing.T) {
	voteSet := NewPrepareVoteSet(newTestLogger(), 1, 1, []byte("digest"), nil)
	// validators为nil，但NewPrepareVoteSet不会返回nil（因为logger不为nil）
	// 但在AddVote时会检查validators是否为nil
	prepare := &pbftpb.Prepare{
		NodeId:   "node1",
		View:     1,
		Sequence: 1,
	...
	require.NotNil(t, err)
	require.False(t, added)
	require.Contains(t, err.Error(), "validators is nil")
}

// TestPrepareVoteSet_AddVote_NilVoteSet 测试nil VoteSet调用AddVote
func TestPrepareVoteSet_AddVote_NilVoteSet(t *testing.T) {
	var nilVoteSet *PrepareVoteSet
	prepare := &pbftpb.Prepare{
		NodeId:   "node1",
		View:     1,
		Sequence: 1,
	...
	added, err := nilVoteSet.AddVote(prepare)
	require.NotNil(t, err)
	require.False(t, added)
	require.Contains(t, err.Error(), "PrepareVoteSet is nil")
}
...
// TestCommitVoteSet_AddVote_NilValidators 测试nil validators的情况
func TestCommitVoteSet_AddVote_NilValidators(t *testing.T) {
	voteSet := NewCommitVoteSet(newTestLogger(), 1, 1, []byte("digest"), nil)
	commit := &pbftpb.Commit{
		NodeId:   "node1",
		View:     1,
		Sequence: 1,
...
// TestCommitVoteSet_AddVote_NilVoteSet 测试nil VoteSet调用AddVote
func TestCommitVoteSet_AddVote_NilVoteSet(t *testing.T) {
	var nilVoteSet *CommitVoteSet
	commit := &pbftpb.Commit{
		NodeId:   "node1",
		View:     1,
		Sequence: 1,
	...
	added, err := nilVoteSet.AddVote(commit)
	require.NotNil(t, err)
	require.False(t, added)
	require.Contains(t, err.Error(), "CommitVoteSet is nil")
}

// TestVoteSet_HasTwoThirdsMajority_NilValidators 测试nil validators的HasTwoThirdsMajority
func TestVoteSet_HasTwoThirdsMajority_NilValidators(t *testing.T) {
	// 创建一个validators为nil的VoteSet（这在实际中不应该发生，但需要测试）
	voteSet := &PrepareVoteSet{
